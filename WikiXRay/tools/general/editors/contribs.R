...
monthlyen=dbGetQuery(conenwiki, "select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(conenwiki)

condewiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_dewiki_research")
monthlyde=dbGetQuery(condewiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(condewiki)

confrwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_frwiki_research")
monthlyfr=dbGetQuery(confrwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(confrwiki)

conplwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_plwiki_research")
monthlypl=dbGetQuery(conplwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(conplwiki)

conjawiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_jawiki_research")
monthlyja=dbGetQuery(conjawiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(conjawiki)

connlwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_nlwiki_research")
monthlynl=dbGetQuery(connlwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(connlwiki)

conitwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_itwiki_research")
monthlyit=dbGetQuery(conitwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(conitwiki)

conptwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_ptwiki_research")
monthlypt=dbGetQuery(conptwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(conptwiki)

coneswiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_eswiki_research")
monthlyes=dbGetQuery(coneswiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
dbDisconnect(coneswiki)

consvwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_svwiki_research")
monthlysv=dbGetQuery(consvwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
...
postscript("./graphics/contribs.eps",onefile=FALSE,horizontal=FALSE,height=9,width=15,paper="special")
plot(enwiki,log="xy",type="b",col=1,ylim=range(1,5e+7),xlab="Time",ylab="Num. of contributions", main="Evolution in time of number of contributions",pch=15,lty=1, cex.axis=1.7, cex.lab=1.5, cex.main=1.8)
grid(col=1)
