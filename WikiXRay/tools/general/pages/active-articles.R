...
# contribs_total=data.frame(theyear=numeric(0), themonth=numeric(0), num_revs=numeric(0), lang=character(0))
# for (alang in langs) {
# 	# Get DB connection
# 	con=dbConnect(MySQL(), user="root", password="phoenix", dbname=paste("wx_",alang,"wiki_research", sep=""))
# 	contribs_monthly=dbGetQuery(con,
# 	"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(*) num_revs from revision group by theyear, themonth")
# 	contribs_monthly=transform(contribs_monthly, lang=alang)
# 	contribs_total=merge(contribs_total, contribs_monthly, all=T)
# 	dbDisconnect(con)
# }

# COUNT MONTHLY NUMBER OF ACTIVE ARTICLES (THOSE RECEIVING AT LEAST ONE REVISION IN THAT MONTH)

conenwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_enwiki_research")
monthlyen=dbGetQuery(conenwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(conenwiki)

condewiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_dewiki_research")
monthlyde=dbGetQuery(condewiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(condewiki)

confrwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_frwiki_research")
monthlyfr=dbGetQuery(confrwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(confrwiki)

conplwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_plwiki_research")
monthlypl=dbGetQuery(conplwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(conplwiki)

conjawiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_jawiki_research")
monthlyja=dbGetQuery(conjawiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(conjawiki)

connlwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_nlwiki_research")
monthlynl=dbGetQuery(connlwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(connlwiki)

conitwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_itwiki_research")
monthlyit=dbGetQuery(conitwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(conitwiki)

conptwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_ptwiki_research")
monthlypt=dbGetQuery(conptwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(conptwiki)

coneswiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_eswiki_research")
monthlyes=dbGetQuery(coneswiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
dbDisconnect(coneswiki)

consvwiki=dbConnect(MySQL(), user="root", password="phoenix", dbname="wx_svwiki_research")
monthlysv=dbGetQuery(consvwiki,"select year(rev_timestamp) theyear, month(rev_timestamp) themonth, count(distinct(rev_page)) articles from rev_main_nored group by theyear, themonth")
...
postscript("./graphics/active-articles.eps",onefile=FALSE,horizontal=FALSE,height=9,width=15,paper="special")
plot(enwiki,log="xy",type="b",col=1,ylim=range(1,5e+7),xlab="Time",ylab="Num. of active articles", main="Evolution in time of number of active articles",pch=15,lty=1, cex.axis=1.7, cex.lab=1.5, cex.main=1.8)
grid(col=1)
