...
for (alang in langs) {
	...
	for (ayear in years) {
		...
		paste("select rev_id, rev_len, ",ayear," as year, \'",alang,"\' as lang from rev_main_nored where rev_id in (select max_id from 
		max_rev_",ayear,")", sep=""))
		dbDisconnect(con)
...
postscript("./graphics/page_length_evol.eps", horizontal=F)
