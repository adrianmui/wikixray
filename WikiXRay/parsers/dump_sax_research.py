...
##            self.outlinks_dict={}; self.trans_dict={}
            ...
            ## Using str.join() instead of plain '+' operator for increased performance
            ...
            newpageinsert="".join(["(",self.page_dict['id'],",",
            self.page_dict['namespace'],",",'"',
            self.page_dict['title'].replace("\\","\\\\").replace("'","\\'").replace('"', '\\"'),
            '"',",",self.prior_rev_id,",",str(self.last_page_len),
            ",",self.isRedirect,",",self.isStub,",",str(random.random())])
            ...
            if self.rev_count>1:
                #newpageinsert+=",0"
                newpageinsert="".join([newpageinsert,",0"])
            else:
                #newpageinsert+=",1"
                newpageinsert="".join([newpageinsert,",1"])
            if self.page_dict.has_key('restrictions'):
                #newpageinsert+=","+'"'+self.page_dict['restrictions']+'"'
                newpageinsert="".join([newpageinsert,",",'"',self.page_dict['restrictions'],'"'])
            else:
                #newpageinsert+=",''"
                newpageinsert="".join([newpageinsert,",''"])
            #newpageinsert+=")"
            newpageinsert="".join([newpageinsert,")"])

            if self.pageinsertrows==0:
                #self.pageinsert="INSERT INTO page VALUES"+newpageinsert
                self.pageinsert="".join(["INSERT INTO page VALUES",newpageinsert])
                self.pageinsertrows+=1
                self.pageinsertsize=len(self.pageinsert)*2
            elif (self.pageinsertrows+1<=self.options.imaxrows) or\
            (self.pageinsertsize+(2*len(newpageinsert))<=self.options.imaxsize*1024):
                #Append current row to extended insert
                #self.pageinsert+=","+newpageinsert
                self.pageinsert="".join([self.pageinsert,",",newpageinsert])
                self.pageinsertrows+=1
                self.pageinsertsize=len(self.pageinsert)*2
            else:
                ...
                if self.options.fileout:
                    ...
                    #self.pageinsert+=";\n"
                    self.pageinsert="".join([self.pageinsert,";\n"])
    ...
    def characters(self, ch):
        if self.current_elem != None:
            ...
            self.current_text = "".join([self.current_text,ch])
...
if __name__ == '__main__':
    ...
    default=50, help="Max size in KB of the MySQL extended inserts [default: %default] "
    "[max: 256]")
    parserc.add_option("--insertmaxnum", dest="imaxrows", metavar="MAXROWS", type="int",
    default=100, help="Max number of individual rows allowed in the MySQL extended "
    "inserts [default: %default][max: 5000]")
    ...
    if options.imaxrows>5000 or options.imaxrows<=0:
